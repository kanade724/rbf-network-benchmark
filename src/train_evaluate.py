...
class HOGFeatureExtractor(BaseEstimator, TransformerMixin):
    def __init__(
        self,
        image_shape: list[int] | tuple[int, int] = (28, 28),
        orientations: int = 9,
        pixels_per_cell: list[int] | tuple[int, int] = (4, 4),
        cells_per_block: list[int] | tuple[int, int] = (2, 2),
    ...
    def fit(self, x: np.ndarray, y: np.ndarray | None = None) -> "HOGFeatureExtractor":
        return self

    def transform(self, x: np.ndarray) -> np.ndarray:
        expected_features = int(np.prod(self.image_shape))
        if x.shape[1] != expected_features:
            raise ValueError(
                f"HOG expects flattened images with {expected_features} features, got {x.shape[1]}."
            )

        features = [
            hog(
                sample.reshape(self.image_shape),
...
def build_preprocessor(config: dict[str, Any]) -> Pipeline:
    ...
    feature_extractor = preprocess.get("feature_extractor", "raw")

    if feature_extractor == "hog":
        hog_config = preprocess.get("hog", {})
        steps.append(
            (
                "hog",
                HOGFeatureExtractor(
                    image_shape=hog_config.get("image_shape", [28, 28]),
                    orientations=int(hog_config.get("orientations", 9)),
                    pixels_per_cell=hog_config.get("pixels_per_cell", [4, 4]),
                    cells_per_block=hog_config.get("cells_per_block", [2, 2]),
                    block_norm=hog_config.get("block_norm", "L2-Hys"),
                ),
            )
        )
    elif feature_extractor != "raw":
        raise ValueError("preprocess.feature_extractor must be 'raw' or 'hog'.")
...
def evaluate_dataset(config: dict[str, Any], dataset: str, run_dir: Path) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "feature_extractor": config.get("preprocess", {}).get("feature_extractor", "raw"),
        "features_after_preprocess": int(x_train.shape[1]),
        "rbf": {
